---
title: "NaRnEA Manuscript - Head and Neck Squamous Cell Carcinoma"
output: html_document
author: Aaron T. Griffin & Lukas J. Vlahos
date: January 12th 2022
vignette: >
  %\VignetteIndexEntry{NaRnEA_HNSC}
...
##### This code is designed to reproduce the analysis from the manuscript "An Information Theoretic Framework for Protein Activity Measurement" for the head and neck squamous cell carcinoma (HNSC) cancer subtype using data from The Cancer Genome Atlas (TCGA) and the Clinical Proteomic Tumor Analysis Consortium (CPTAC).
...
##### The head and neck squamous cell carcinoma gene expression data is publicly available from The Cancer Genome Atlas (TCGA). This data was downloaded using the `TCGAbiolinks` R package, and gene names were converted to ENTREZ using the `biomaRt` R package. The matrix is available as a `.rds` file within the NaRnEA package. We also load in a mapping between different gene name conventions generated using `biomaRt`:

```{r include = TRUE, echo = TRUE, eval = FALSE}
data(TCGA_HNSC)
data(gene.name.map)
cur.tcga.entrez.counts.mat <- TCGA_HNSC
```
...
saveRDS(cur.paired.tumor.counts.mat, file = paste(cur.output.dir, '/TCGA_HNSC_Paired_Tumor_Counts_Mat.rds', sep = '')) 
