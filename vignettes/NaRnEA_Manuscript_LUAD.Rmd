...
vignette: >
  %\VignetteIndexEntry{NaRnEA_Manuscript_LUAD}
...
##### This code is designed to reproduce the analysis from the manuscript for Nonparametric analytical Rank-based Enrichment Analysis (`NaRnEA`) for the lung adenocarcinoma (LUAD) cancer subtype using data from The Cancer Genome Atlas (TCGA) and the Clinical Proteomic Tumor Analysis Consortium (CPTAC).
...
##### The lung adenocarcinoma gene expression data is publicly available from The Cancer Genome Atlas (TCGA). We can use the `TCGAbiolinks` R package to download the data and prepare the counts matrix for analysis.

```{r include = TRUE, echo = TRUE, eval = FALSE}
# use the TCGAbiolinks R package to load in the count-based gene expression profiles for the TCGA LUAD cohort and the associated metadata
cur.project.name <- "TCGA-LUAD"
...
saveRDS(object = cur.tcga.entrez.counts.mat, file = paste(cur.output.dir,"/TCGA_LUAD_Entrez_Counts_Mat.rds", sep = "")) 
```
...
saveRDS(object = cur.paired.tumor.counts.mat, file = paste(cur.output.dir,"/TCGA_LUAD_Paired_Tumor_Counts_Mat.rds", sep = "")) 
...
##### We can directly inspect the agreement between the `NaRnEA`-inferred ledge precision values and `ARACNe3`-inferred Regulation Confidence values for six regulon gene sets which are significantly enriched in the differential gene expression signature between TCGA LUAD primary tumor and adjacent normal tissue. These regulon gene sets were selected because the corresponding putative transcriptional regulatory proteins are either positively associated with tumorigenesis (i.e. FOXM1, UHRF1, MYBL2) or because they are known to function as tumor suppressors (i.e. TCF21, TBX4, AFF3). In addition to visualizing the agreement between the `NaRnEA`-inferred ledge precision values and the `ARACNe3`-inferred Regulation Confidence values with a scatterplot, we can use the `EnrichmentPlot` function in the `NaRnEA` R package to directly visualize the enrichment of each `ARACNe3`-inferred regulon gene set in the nonparametrically transformed differential gene expression signature along with the corresponding `NaRnEA` Proportional Enrichment Score, Normalized Enrichment Score, and two-sided p-value. The `EnrichmentPlot` function can also be used to etimate a 95% confidence interval for the `NaRnEA` Proportional Enrichment Score using regulon gene set member bootstrapping and Fisher's Transformation. We see that the `NaRnEA`-inferred regulon gene set enrichment for each putative transcriptional regulatory protein is highly statistically significant and directionally in agreement with the known role these proteins play in tumorigenesis or tumor suppression, respectively.  

```{r include = TRUE, echo = TRUE, eval = FALSE}
# visualize the gene set enrichment and agreement between the NaRnEA-inferred ledge precision values and the ARACNe3-inferred Regulation Confidence values for FOXM1, UHRF1, MYBL2, TCF21, TBX4, and AFF3
for(sub.hugo.name in c("FOXM1", "UHRF1", "MYBL2", "TCF21", "TBX4", "AFF3")){
	
	sub.reg.name <- sub.gene.conversion.data[match(sub.hugo.name,as.character(sub.gene.conversion.data$hugo.values)),"entrez.values"]
	
	sub.rc.values <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$rc.values
	names(sub.rc.values) <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$target.values
	
	sub.mor.values <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$mor.values
	names(sub.mor.values) <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$target.values
	
	sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")
	
	print(sub.plot)
	
	ggsave(filename = paste("Fig_", figure.output.value, "_3_", sub.hugo.name, "_Regulon_Enrichment_Plot.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
	
	sub.plot.data <- data.frame(x.values = cur.aracne.narnea.ledge.ppv.vs.rc.list[[match(sub.reg.name, names(cur.aracne.narnea.ledge.ppv.vs.rc.list))]]$rc.values, y.values = cur.aracne.narnea.ledge.ppv.vs.rc.list[[match(sub.reg.name, names(cur.aracne.narnea.ledge.ppv.vs.rc.list))]]$ledge.ppv.values)
	
	sub.scc.res <- cor.test(x = rank(sub.plot.data$x.values, ties.method = "random"), y = rank(sub.plot.data$y.values, ties.method = "random"))
	
	sub.color.choice <- "red1"
	if(cur.aracne3.narnea.res[[match(sub.reg.name,names(cur.aracne3.narnea.res))]]$pes < 0){
		sub.color.choice <- "blue1"
	}
	...
	cur.title <- "NaRnEA Manuscript - LUAD"
	cur.subtitle <- paste("Regulator = ", sub.hugo.name, " : ", "PES = ", signif(sub.scc.res.data[match(sub.reg.name, rownames(sub.scc.res.data)),"pes.values"], sig.figs), " : ", "NES = ", signif(sub.scc.res.data[match(sub.reg.name, rownames(sub.scc.res.data)),"nes.values"], sig.figs), " : ", "p = ", signif(sub.scc.res.data[match(sub.reg.name, rownames(sub.scc.res.data)),"p.values"], sig.figs), "\n", "Spearman Correlation = ", signif(sub.scc.res$estimate, sig.figs), " [", signif(sub.scc.res$conf.int[1], sig.figs),",", signif(sub.scc.res$conf.int[2], sig.figs),"] (p = ",signif(sub.scc.res$p.value, sig.figs),")", sep = "")
	cur.x.lab <- "ARACNe3-Inferred Regulation Confidence"
	cur.y.lab <- "NaRnEA-Inferred Ledge Precision"
	
	sub.plot <- ggplot(sub.plot.data) + theme_bw() + geom_hline(colour = "black", lwd = 1, yintercept = range(y.breaks)) + geom_vline(colour = "black", lwd = 1, xintercept = range(x.breaks)) + geom_point(aes(x = x.values, y = y.values), colour = sub.color.choice, shape = 21, size = 3, fill = "white", stroke = 1, alpha = 1) + scale_x_continuous(limits = range(x.breaks), breaks = x.breaks) + scale_y_continuous(limits = range(y.breaks), breaks = y.breaks) + labs(title = cur.title, subtitle = cur.subtitle, x = cur.x.lab, y = cur.y.lab) + theme(plot.title = element_text(hjust = 0.5, colour = "black", size = 30), plot.subtitle = element_text(hjust = 0.5, colour = "black", size = 23), axis.title = element_text(hjust = 0.5, colour = "black", size = 25), axis.text.x = element_text(hjust = 0.5, colour = "black", size = 22), axis.text.y = element_text(hjust = 1, colour = "black", size = 22), legend.title = element_text(hjust = 0, colour = "black", size = 20), legend.text = element_text(hjust = 0, colour = "black", size = 18), legend.position = "bottom")	
	
	print(sub.plot)
	
	ggsave(filename = paste("Fig_", figure.output.value, "_4_", sub.hugo.name, "_PPV_RC_Scatterplot.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 12, width = 12)

}
```

![](../man/figures/Fig_1_3_FOXM1_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_FOXM1_PPV_RC_Scatterplot.png)
![](../man/figures/Fig_1_3_UHRF1_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_UHRF1_PPV_RC_Scatterplot.png)
![](../man/figures/Fig_1_3_MYBL2_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_MYBL2_PPV_RC_Scatterplot.png)
![](../man/figures/Fig_1_3_TCF21_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_TCF21_PPV_RC_Scatterplot.png)
![](../man/figures/Fig_1_3_TBX4_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_TBX4_PPV_RC_Scatterplot.png)
![](../man/figures/Fig_1_3_AFF3_Regulon_Enrichment_Plot.png)
![](../man/figures/Fig_1_4_AFF3_PPV_RC_Scatterplot.png)

### Explore How Gene Set Parameters Influence Enrichment

##### Finally, we explore how changes to gene set Regulation Confidence and Mode of Regulation values can alter `NaRnEA`-inferred gene set enrichment. This serves as both an encouragement to the user that they should carefully tune these values based on *a priori* knowledge to maximize statistical power; additionally, this serves as a warning that the use of *a posteriori* knowledge to tune these parameters can grossly inflate the Type I error rate of gene set analysis.

##### We begin with the `ARACNe3`-inferred regulon gene set for FOXM1 and test for the enrichment of the regulon gene set in the differential gene expression signature between TCGA LUAD paired primary tumor and adjacent normal tissue gene expression profiles. We observe statistically significant, positive enrichment of the regulon gene set.

```{r include = TRUE, echo = TRUE, eval = FALSE}
# set the seed for the random number generator
set.seed(sim.seed)

# select the ARACNe3-inferred FOXM1 regulon gene set and visualize the enrichment without any modification to the Regulation Confidence or Mode of Regulation values
sub.hugo.name <- "FOXM1"
...
sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V1)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_5.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```

![](../man/figures/Fig_1_5.png)

##### We can examine the importance of the `ARACNe3`-inferred Regulation Confidence values by testing for enrichment of the FOXM1 regulon gene set after equalizing all Regulation Confidence values (i.e. setting them all equal to one). This results in a decrease in the `NaRnEA` Proportional Enrichment Score for the regulon gene set enrichment, which reiterates the previous finding that `ARACNe3`-inferred Regulation Confidence values are positively associated with the extent to which each regulon gene set member contributes to the gene set enrichment. 

```{r include = TRUE, echo = TRUE, eval = FALSE}
# modify the ARACNe3-inferred FOXM1 regulon gene set so that all Regulation Confidence values are equal
set.seed(sim.seed)
sub.hugo.name <- "FOXM1"

sub.reg.name <- sub.gene.conversion.data[match(sub.hugo.name,as.character(sub.gene.conversion.data$hugo.values)),"entrez.values"]

sub.rc.values <- rep(1, times = length(aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$rc.values))
names(sub.rc.values) <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$target.values

sub.mor.values <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$mor.values
names(sub.mor.values) <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$target.values

sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V2)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_6.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```


![](../man/figures/Fig_1_6.png)

##### In addition to equalizing the Regulation Confidence values, we can also alter the Mode of Regulation values by setting them all close to one using a very narrow uniform distribution. The result of this is an even greater decrease in the gene set enrichment as we lose the crucial regulatory information for much of the regulon gene set. Thus, the power of gene set analysis with `NaRnEA` depends on accurate tuning of both the Regulation Confidence and the Mode of Regulation. 

```{r include = TRUE, echo = TRUE, eval = FALSE}
# modify the ARACNe3-inferred FOXM1 regulon gene set so that all Regulation Confidence values are equal and all Mode of Regulation values are nearly one
set.seed(sim.seed)
sub.hugo.name <- "FOXM1"
...
sub.mor.values <- runif(n = length(aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$mor.values), min = 0.99, max = 1)
names(sub.mor.values) <- aracne3.network.data.list[[match(sub.reg.name, names(aracne3.network.data.list))]]$target.values

sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V3)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_7.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```

![](../man/figures/Fig_1_7.png)

##### Conversely, we can examine how tuning the null FOXM1 gene set with *a posteriori* data can inflate the Type I error rate of gene set analysis. We begin by testing for the enrichment of the null FOXM1 gene set without any changes and find that it is not significantly enriched in the differential gene expression signature. 

```{r include = TRUE, echo = TRUE, eval = FALSE}
# select the NULL FOXM1 regulon gene set and visualize the enrichment without any modification to the Regulation Confidence or Mode of Regulation values
sub.hugo.name <- "FOXM1"

sub.reg.name <- sub.gene.conversion.data[match(sub.hugo.name,as.character(sub.gene.conversion.data$hugo.values)),"entrez.values"]

sub.rc.values <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$rc.values
names(sub.rc.values) <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$target.values

sub.mor.values <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$mor.values
names(sub.mor.values) <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$target.values

sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V4)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_8.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```

![](../man/figures/Fig_1_8.png)

##### We can modify the null FOXM1 gene set *a posteriori* by setting the Mode of Regulation for each gene set member based on the sign of the differential gene expression value of each gene set member; in essence, this produces a strongly overfit gene set which demonstrates substantial enrichment in the differential gene expression signature despite the fact that the gene set members were chosen completely at random and have no biological relationship. 

```{r include = TRUE, echo = TRUE, eval = FALSE}
# select the NULL FOXM1 regulon gene set and modify the Mode of Regulation based on the differential gene expression of each gene set member 
sub.hugo.name <- "FOXM1"
...
sub.mor.values <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$mor.values
names(sub.mor.values) <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$target.values
sub.mor.values <- sign(cur.tumor.vs.tissue.mwu.ges[match(names(sub.mor.values), names(cur.tumor.vs.tissue.mwu.ges))])

sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V5)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_9.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```

![](../man/figures/Fig_1_9.png)

##### Additionally, we can modify the null FOXM1 gene set *a posteriori* by setting the Regulation Confidence for each gene set member based on the magnitude of the differential gene expression signature value of each gene set member; this nearly doubles the `NaRnEA` Proportional Enrichment Score and results in even more drastic overfitting of the gene set to the differential gene expression signature despite the fact that the gene set members were chosen completely at random and have no biological relationship. 

```{r include = TRUE, echo = TRUE, eval = FALSE}
# select the NULL FOXM1 regulon gene set and modify the Mode of Regulation and Regulation Confidence based on the differential gene expression of each gene set member 
sub.hugo.name <- "FOXM1"
...
sub.rc.values <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$rc.values
names(sub.rc.values) <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$target.values
sub.rc.values <- abs(cur.tumor.vs.tissue.mwu.ges[match(names(sub.rc.values), names(cur.tumor.vs.tissue.mwu.ges))])
sub.rc.values <- rank(sub.rc.values, ties.method = "random")/length(sub.rc.values)

sub.mor.values <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$mor.values
names(sub.mor.values) <- null.network.data.list[[match(sub.reg.name, names(null.network.data.list))]]$target.values
sub.mor.values <- sign(cur.tumor.vs.tissue.mwu.ges[match(names(sub.mor.values), names(cur.tumor.vs.tissue.mwu.ges))])

sub.plot <- EnrichmentPlot(signature = cur.tumor.vs.tissue.mwu.ges, regulation.confidence = sub.rc.values, mode.of.regulation = sub.mor.values, title.text = paste("NaRnEA-Inferred ", sub.hugo.name, " Regulon Gene Set Enrichment (V6)", sep = ""), x.label.text = "Nonparametric TCGA LUAD Tumor vs. Tissue Gene Expression Signature")

print(sub.plot)

ggsave(filename = paste("Fig_", figure.output.value, "_10.png", sep = ""), dpi = 500, path = "PNG_Figures", device = "png", height = 2, width = 12)
```
