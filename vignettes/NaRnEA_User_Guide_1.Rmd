...
```{r, include = FALSE}
library(knitr)
opts_chunk$set(collapse = TRUE, comment = "#>", tidy = TRUE, tidy.opts=list(width.cutoff=55))
```
...
Because the members of this gene set were selected at random from the gene expression signature we know the null hypothesis of gene set analysis is true for this gene set (i.e. the gene set is not enriched in the gene expression signature). Let's check to see if NaRnEA agrees with this ground truth; we can test this numerically by calling the `NaRnEA` function directly or by using the `EnrichmentAnalysisPlot` visualization function.
```{r, eval = FALSE}
...
```
```{r, echo = FALSE}
knitr::include_graphics(path = "../man/figures/NaRnEA_User_Guide_1_Fig_31.png", dpi = 500)
```

We see that the NaRnEA normalized enrichment score steadily increases when the gene set size is increased while the NaRnEA proportional enrichment score remains stable. The 95% non-parametric confidence interval for the NaRnEA proportional enrichment score becomes smaller as the gene set size increases. All of this behavior agrees well with the null hypothesis significance testing framework. Larger gene sets have a greate statistical power to reject the null hypothesis, which is correctly reflected in the size-dependence of the normalized enrichment score. Larger gene sets also afford a greater degree of precision when we estimate the proportional enrichment score, which is correctly reflected in the inverse dependence between the proportional enrichment score confidence interval width and the gene set size. As with all frequentist tests, both the test statistic (i.e. normalized enrichment score) and the effect size (i.e. proportional enrichment score) must be considered when evaluating the output of NaRnEA.
